      ...
      <div className="relative px-4 sm:px-8 py-6 sm:py-8">
        <div className="max-w-[1600px] mx-auto">
          {projects.length > 0 ? (
              ...
              <AnimatePresence>
                {projects.map((project, index) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1, transition: { delay: index * 0.05, duration: 0.4, ease: [0.22, 1, 0.36, 1] } }}
                    exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.2 } }}
                    whileHover={{ y: -8, transition: { duration: 0.3, ease: [0.22, 1, 0.36, 1] } }}
                    onClick={() => navigate(`/projects/${project._id}`)}
                    // card padding p-3
                    className="group relative bg-white border border-slate-200/80 rounded-2xl p-3 cursor-pointer hover:border-sky-300/60 transition-all duration-300 shadow-md hover:shadow-2xl hover:shadow-sky-500/10 overflow-hidden"
                  >
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      ...
                      <div className="flex items-start justify-between mb-3 flex-1">
                        <div className="flex-1 min-w-0 pr-3">
                          ...
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            <h3 className="text-lg font-bold text-slate-900 group-hover:text-sky-600 transition-colors duration-300 truncate">{project.name}</h3>
                          </div>
                        ...
                        <div className="relative" ref={showMenu === project._id ? menuRef : null}>
                          <motion.button
                            ...
                            whileTap={{ scale: 0.9 }}
                            onClick={(e) => { e.stopPropagation(); setShowMenu(showMenu === project._id ? null : project._id); }}
                          ...
                          <AnimatePresence>
                            {showMenu === project._id && (
                              <motion.div
                                ...
                                className="absolute top-full right-0 mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden"
                                onClick={(e) => e.stopPropagation()}
                              >
                                <div className="py-1">
                                  <motion.button whileHover={{ x: 4, backgroundColor: 'rgb(248 250 252)'}} onClick={() => handleOpenEditModal(project)} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left text-slate-700 font-medium">
                                    <Edit2 className="w-4 h-4" /><span>Edit Project</span>
                                  </motion.button>
                                  <div className="h-px bg-slate-100 mx-2" />
                                  <motion.button whileHover={{ x: 4, backgroundColor: 'rgb(254 242 242)'}} onClick={() => { setDeleteConfirm(project); setShowMenu(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 text-left font-medium">
                      ...
                      {/* top padding pt-3 */}
                      <div className="flex items-center justify-between pt-3 border-t border-slate-100 group-hover:border-sky-100 transition-colors">
                        <div className="flex items-center gap-4 text-xs font-medium">
                          <div className="flex items-center gap-1.5 text-slate-500"><Calendar className="w-3.5 h-3.5" /><span>{new Date(project.createdAt).toLocaleDateString()}</span></div>
                          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 rounded-lg text-slate-600"><FileText className="w-3.5 h-3.5" /><span>{project.taskCount ?? 0}</span></div>
                        </div>
                        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center py-20 sm:py-32">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 text-center max-w-md">
                <h3 className="text-2xl font-bold text-slate-800 mb-2">No Projects Yet</h3>
                <p className="text-slate-600 text-base leading-relaxed mb-6">Start your journey by creating your first project. Organize your tasks and boost your productivity.</p>
                <motion.button whileHover={{ scale: 1.05, y: -2 }} onClick={onNewProject} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 text-white font-semibold shadow-lg">
                  <Plus className="w-5 h-5" /><span>Create Your First Project</span>
                </motion.button>
              </motion.div>
            </motion.div>
          )}
